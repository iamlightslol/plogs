...
class User_model extends CI_Model {
  ...
  public function __construct(){
	...
	{
			$orgnization_id=$this->input->post('org_id');
			$rto=$this->input->post('rotation');
			$resident_year=$this->input->post('resident_year');
				
			$date=date("d/m/Y",strtotime($this->input->post('date')));
			$patient_age=$this->input->post('patient_age');
			$age=$this->input->post('age');
			$new_age=$patient_age." ".$age;
			$newdate=implode('/', array_reverse(explode('/', $date)));
			$months=date("m",strtotime($newdate));
			
			if($age=='Year'){$age_in_year=$patient_age;}
			else{$age_in_year="0";}
			...
			$scnd=$this->input->post('secondary_diagnoses');
			for($i=1;$i<=$total;$i++)
			{
				$sec_diagnoses=$this->input->post('secondary_diagnoses'.$i);
				if($sec_diagnoses!='')
			...
			if(@$scnd_dgns[0]!='' && $scnd!=''){$b=$scnd.", ".implode(", ", @$scnd_dgns); }
			else if($scnd!=''){$b=$scnd; }
			...
			$data=array(
			'patient_unique_id'=>$randomString,
			'resident_id'=>$this->input->post('resident_id'),
			'resident_user'=>$this->input->post('resident_user'),
			'residency_year'=>$resident_year,
			'org_id'=>$orgnization_id,
			'doj'=>$newdate,
			'fld_cur_time'=>$this->input->post('fld_cur_time'),
			'patient_number'=>$this->input->post('patient_number'),
			'months'=>$months,
			'years'=>$new_age,
			'age_in_year'=>$age_in_year,
	...
	public function get_unique_date($r_id,$res_year)
			{
	...
	{
   				$query=$this->db->query("SELECT distinct patient_unique_id
FROM patient_logs where doj='".$date."' and resident_id='".$id."'
group by patient_unique_id
order by max(str_to_date(fld_cur_time,'%h:%i %p')) desc");
	...
	public function get_total_encounter_data($id,$date,$name)
	{
		...
		$this->db->where('patient_unique_id', $name);
        $query = $this->db->get('patient_logs');
		return $query->num_rows;
		}
		else
...
FROM patient_logs where patient_unique_id='".$name."' and doj='".$date."' and resident_id='".$id."' ORDER BY STR_TO_DATE(fld_cur_time,'%h:%i %p') desc limit 1");
				return $query->result();
	...
	public function get_remaining_data($id,$name,$date,$limit)
	{
        ...
        $query=$this->db->query("SELECT * 
FROM patient_logs where patient_unique_id='".$name."' and doj='".$date."' and resident_id='".$id."' ORDER BY STR_TO_DATE(fld_cur_time,'%h:%i %p') desc limit 1,$limit");
				return $query->result();
	}
